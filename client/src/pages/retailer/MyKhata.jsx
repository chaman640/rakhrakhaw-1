    ...
    <>
      ...
      <Card className={`mb-5 ${due > 0 ? 'border-amber-200 bg-amber-50/50' : 'border-emerald-200 bg-emerald-50/40'}`}>
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            {data.openInvoices?.length > 0 && (
              <p className="mt-1 text-xs text-slate-500">
                {data.openInvoices.length} bill ke paise baaki hain
              </p>
            )}
          </div>
          ...
          {due > 0.01 && (
            data.upi ? (
              <Button size="lg" icon={Smartphone} onClick={() => setPayOpen(true)}>Paisa bhejein</Button>
            ) : (
              <p className="max-w-56 rounded-lg bg-white px-3 py-2 text-xs text-slate-500">
      ...
      {data.openInvoices?.length > 0 && (
        <Card className="mb-5" padding={false}>
          <CardHeader className="p-5 pb-0" title="Ye bill baaki hain"
            subtitle="Paisa bhejenge to sabse purana bill pehle clear hoga" />
          <div className="mt-2">
            {data.openInvoices.map((inv) => (
              <button key={inv._id} onClick={() => navigate(`/my-bills/${inv._id}`)}
                ...
                <div className="shrink-0 text-right">
                  ...
                  {inv.paidAmount > 0 && (
                    <p className="text-[11px] text-slate-400">{formatMoney(inv.paidAmount)} de chuke</p>
                  )}
                </div>
      ...
      {payments.length > 0 && (
        <Card className="mb-5" padding={false}>
          <CardHeader className="p-5 pb-0" title="Maine jo bheja" />
          <div className="mt-2">
            {payments.map((p) => {
        ...
        <div className="flex flex-wrap items-end gap-3 border-b border-slate-100 p-4">
          ...
          {(from || to) && (
            <Button variant="ghost" size="sm" icon={RotateCcw}
              onClick={() => { setFrom(''); setTo(''); }}>Hatayein</Button>
          )}
        </div>

        {!loading && !data.entries?.length ? (
          <EmptyState icon={BookOpen} title="Abhi koi hisaab nahi"
            message="Pehla bill banega tab yahan dikhega." />
        ) : (
    ...
    <Modal
      ...
      footer={step === 1 ? (
        <>
          ...
          <Button onClick={() => setStep(2)} disabled={!(Number(amount) > 0)}>
            Bhej diya, aage badhein
          </Button>
        </>
      ) : (
        <>
          <Button variant="secondary" onClick={() => setStep(1)}>Peeche</Button>
          <Button onClick={markSent} loading={saving}>Haan, bhej diya</Button>
        </>
      )}
    >
      {step === 1 ? (
        <div className="space-y-4">
          <Input
            label="Kitna bhejna hai" required type="number" min="0" step="0.01" prefix="₹"
          ...
          {qr && (
            <div className="flex flex-col items-center rounded-xl border border-slate-200 bg-slate-50 p-4">
              <img src={qr} alt="UPI QR code — scan karke paisa bhejein"
                className="h-44 w-44 rounded-lg bg-white p-1.5" />
              <p className="mt-2 text-xs text-slate-500">Kisi bhi UPI app se scan karein</p>
            </div>
          )}
          ...
          <div className="flex items-center gap-2 rounded-lg border border-slate-200 px-3 py-2.5">
            ...
            <button onClick={copyId}
              ...
              {copied ? <><Check size={13} /> Copy hua</> : <><Copy size={13} /> Copy</>}
            </button>
          </div>
          ...
          <a href={link}
            ...
            <Smartphone size={16} /> UPI app kholein
          </a>
        ...
        <div className="space-y-4">
          <div className="rounded-lg bg-emerald-50 px-3 py-2.5 text-sm text-emerald-900">
            {formatMoney(Number(amount) || 0)} bhejne ki baat likh rahe hain
          </div>

          <Input
            label="UPI transaction / UTR number"
            value={reference} onChange={(e) => setReference(e.target.value)}
            placeholder="4xxxxxxxxxxx"
            hint="App me 'transaction ID' likha milega — daal denge to confirm jaldi hoga"
          />

          <Textarea label="Kuch kehna ho to" rows={2} value={note}
            onChange={(e) => setNote(e.target.value)}
            placeholder="Jaise: bill RB/26-27/0004 ka paisa" />

          <p className="rounded-lg bg-amber-50 px-3 py-2 text-xs text-amber-900">
            Ye abhi khate me nahi lagega. Wholesaler apna account dekh kar confirm karega, tabhi
            aapka udhaar kam hoga.
          </p>
