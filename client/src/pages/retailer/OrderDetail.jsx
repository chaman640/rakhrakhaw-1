  ...
  return (
    <>
      {isNew && (
        <Card className="mb-5 border-emerald-200 bg-emerald-50">
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="text-sm font-medium text-emerald-900">Order chala gaya!</p>
              <p className="mt-0.5 text-sm text-emerald-800">
                {business?.name || 'Wholesaler'} ko aapka order mil gaya hai. Tayyar hote hi aapko yahin
                status dikh jayega.
              </p>
            </div>
          </div>
      ...
      <Card className="mb-5">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">
              {formatDateTime(order.orderDate || order.createdAt)} · {order.itemCount} item
            </p>
          </div>
          ...
          <div className="flex flex-wrap gap-2">
            {order.invoiceId && (
              <Link to={`/my-bills/${order.invoiceId}`}>
                <Button variant="secondary" size="sm" icon={Receipt}>Bill dekhein</Button>
              </Link>
            )}
            {order.status === 'PLACED' && (
              <Button variant="secondary" size="sm" icon={XCircle} onClick={() => setConfirm(true)}>
                Order cancel karein
              </Button>
            )}
          </div>
        ...
        {cancelled && (
          <p className="mt-4 rounded-lg bg-red-50 px-3 py-2.5 text-sm text-red-800">
            Ye order cancel ho gaya tha{order.cancelReason ? ` — ${order.cancelReason}` : ''}.
          </p>
        )}

        {order.retailerNote && (
          <p className="mt-4 rounded-lg bg-slate-50 px-3 py-2.5 text-sm text-slate-700">
            Aapka note: {order.retailerNote}
          </p>
        )}
      </Card>
      ...
      <ConfirmModal
        ...
        message="Ye order wapas nahi aayega. Zarurat ho to naya order kar sakte hain."
