          ...
          <div className="rounded-lg border border-slate-200 p-4">
            ...
            {s.newCategories?.length > 0 && (
              <p className="mt-3 rounded-lg bg-blue-50 px-3 py-2 text-sm text-blue-800">
                {s.newCategories.length} nayi category bhi ban jayegi: {s.newCategories.join(', ')}
              </p>
            )}
            ...
            {errorRows.length > 0 ? (
              <div className="mt-4">
                <p className="mb-2 flex items-center gap-1.5 text-sm font-medium text-red-700">
                  <AlertTriangle size={15} /> Ye rows skip ho jayengi
                </p>
                <ul className="max-h-48 divide-y divide-red-100 overflow-y-auto rounded-lg border border-red-200 bg-red-50">
                  {errorRows.slice(0, 20).map((r) => (
                    <li key={r.line} className="px-3 py-2 text-sm">
                      <span className="font-medium text-red-900">Line {r.line}</span>
                ...
                {errorRows.length > 20 && (
                  <p className="mt-1 text-xs text-slate-500">...aur {errorRows.length - 20} rows</p>
                )}
              </div>
            ) : (
              <p className="mt-3 flex items-center gap-1.5 rounded-lg bg-emerald-50 px-3 py-2 text-sm text-emerald-800">
                <CheckCircle2 size={15} /> Saari rows sahi hain
              </p>
            )}
